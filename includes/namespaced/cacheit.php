...
add_action( 'plugins_loaded', function () {
	if ( defined( 'PUM_CACHEIT_COUNTS' ) && PUM_CACHEIT_COUNTS && class_exists( 'QM_Collectors' ) ) {
		// Register collector
...
function track_value_history( $fn_name, $args, $value ) {
	if ( ! defined( 'PUM_CACHEIT_VALUE_HISTORY' ) || ! PUM_CACHEIT_VALUE_HISTORY ) {
		return;
	}
	...
	// Set up history.
	if ( ! isset( $value_history[ $key ] ) ) {
		$value_history[ $key ] = [];
	}

	// If the value has already been logged, don't log it again.
	if ( isset( $value_history[ $key ][0] ) && $value_history[ $key ][0]['value'] === $value ) {
		return;
	}

	// Get current hook info.
	$current_filter = current_filter();

	// Store value with context.
	$value_history[ $key ][] = [
		'value'     => $value,
		'hook'      => $current_filter ?: 'none',
