...
function hasUnreleasedChanges() {
	...
	try {
		...
		const hasContent = unreleasedSection.split( '\n' ).some( ( line ) => {
...
function main() {
	log(
		`${ colors.bold }🚀 Popup Maker Package Bump & Publish${ colors.reset }`
	);
	...
	if ( versionType === 'patch' ) {
		...
		if ( ! hasChanges ) {
			...
			warning(
				'🚫 Patch releases require unreleased changes to justify the release'
			);
			log(
				'\n💡 Patch releases are for quick fixes that need to go out immediately.'
			);
			log(
				'   For planned releases, use minor or major version bumps instead.'
			);
			log(
		...
		info(
			'✅ Found unreleased changes in CHANGELOG.md - proceeding with patch release'
		);
	}
...
`;
	...
	if ( publishedCount > 0 ) {
		...
		// Tag the release
		const tagName = `packages-${ versionType }-${
			new Date().toISOString().split( 'T' )[ 0 ]
		}`;
		execCommand(
			`git tag -a ${ tagName } -m "Package ${ versionType } version bump"`
		);
