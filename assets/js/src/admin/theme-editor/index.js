...
( function ( $ ) {
	...
	PUM_Admin.themeEditor = {
		preview_fixed_scroll: function () {
			...
			$( window ).on( 'scroll', function () {
				if (
					$( '> .postbox:visible', $parent ).index( $preview ) ===
						$( '> .postbox:visible', $parent ).length - 1 &&
		...
		update_font_options: function ( prefix ) {
			var $font_family_select = $(
					'select[id="' + prefix + '_font_family"]'
				),
				$font_weight_select = $(
					'select[id="' + prefix + '_font_weight"]'
				),
				$font_style_select = $(
					'select[id="' + prefix + '_font_style"]'
				),
				...
				if ( font.variants.length ) {
					for ( i = 0; font.variants.length > i; i += 1 ) {
						if ( font.variants[ i ] === 'regular' ) {
							...
							$( 'option[value=""]', $font_style_select ).prop(
								'disabled',
								false
							);
						} else {
			...
			if (
				chosen_weight === '' ||
				$font_weight_options
					.filter( '[value="' + chosen_weight + '"]' )
					.is( ':disabled' )
			) {
				for ( i = 0; font_weights.length > i; i += 1 ) {
					if (
						! $font_weight_options
							.filter( '[value="' + font_weights[ i ] + '"]' )
							.is( ':disabled' )
					) {
			...
			if (
				$font_style_options
					.filter( '[value="' + chosen_style + '"]' )
					.is( ':disabled' )
			) {
				for ( i = 0; font_styles.length > i; i += 1 ) {
					if (
						! $font_style_options
							.filter( '[value="' + font_styles[ i ] + '"]' )
							.is( ':disabled' )
					) {
		...
		update_font_selectboxes: function () {
			return $( 'select[id$="_font_family"]' ).each( function () {
				PUM_Admin.themeEditor.update_font_options(
					$( this ).attr( 'id' ).replace( '_font_family', '' )
				);
			} );
		},
		update_loaded_font: function ( prefix ) {
			var $font_family_select = $(
				...
				$new_link = $(
					'<link id="pum-' +
						prefix +
						'google-font" rel="stylesheet" type="text/css">'
				),
				url;
		...
		update_loaded_fonts: function () {
			return $( 'select[id$="_font_family"]' ).each( function () {
				PUM_Admin.themeEditor.update_loaded_font(
					$( this ).attr( 'id' ).replace( '_font_family', '' )
				);
			} );
		},
			...
			if ( typeof window.PUM_ATB !== 'undefined' ) {
				...
				for ( var old_setting in form_values ) {
					if ( form_values.hasOwnProperty( old_setting ) )
						theme_settings[
							old_setting.replace( 'popup_theme_', '' )
						] = form_values[ old_setting ];
				}
			}
		...
		restyle_preview: function ( theme ) {
			var $overlay = $( '.pum-popup-overlay' ),
				...
				container_inset =
					theme.container_boxshadow_inset === 'yes' ? 'inset ' : '',
				close_inset =
					theme.close_boxshadow_inset === 'yes' ? 'inset ' : '',
				top,
				left,
				right,
				bottom;
			...
			$overlay.removeAttr( 'style' ).css( {
				backgroundColor: window.PUM_Admin.utils.convert_hex(
					theme.overlay_background_color,
					theme.overlay_background_opacity
				),
			} );
			$container.removeAttr( 'style' ).css( {
				...
				backgroundColor: window.PUM_Admin.utils.convert_hex(
					...
					theme.container_background_opacity
				),
				...
				boxShadow:
					container_inset +
					theme.container_boxshadow_horizontal +
					'px ' +
					theme.container_boxshadow_vertical +
					'px ' +
					theme.container_boxshadow_blur +
					'px ' +
					theme.container_boxshadow_spread +
					'px ' +
					window.PUM_Admin.utils.convert_hex(
						theme.container_boxshadow_color,
						theme.container_boxshadow_opacity
					),
			} );
			$title.removeAttr( 'style' ).css( {
				...
				textShadow:
					theme.title_textshadow_horizontal +
					'px ' +
					theme.title_textshadow_vertical +
					'px ' +
					theme.title_textshadow_blur +
					'px ' +
					window.PUM_Admin.utils.convert_hex(
						theme.title_textshadow_color,
						theme.title_textshadow_opacity
					),
			} );
			...
			$close.removeAttr( 'style' ).css( {
				...
				height:
					theme.close_height > 0 ? theme.close_height + 'px' : 'auto',
				width:
					theme.close_width > 0 ? theme.close_width + 'px' : 'auto',
				backgroundColor: window.PUM_Admin.utils.convert_hex(
					theme.close_background_color,
					theme.close_background_opacity
				),
				...
				boxShadow:
					close_inset +
					theme.close_boxshadow_horizontal +
					'px ' +
					theme.close_boxshadow_vertical +
					'px ' +
					theme.close_boxshadow_blur +
					'px ' +
					theme.close_boxshadow_spread +
					'px ' +
					window.PUM_Admin.utils.convert_hex(
						theme.close_boxshadow_color,
						theme.close_boxshadow_opacity
					),
				textShadow:
					theme.close_textshadow_horizontal +
					'px ' +
					theme.close_textshadow_vertical +
					'px ' +
					theme.close_textshadow_blur +
					'px ' +
					window.PUM_Admin.utils.convert_hex(
						theme.close_textshadow_color,
						theme.close_textshadow_opacity
					),
			} );

			// Escape the close text.
			theme.close_text = theme.close_text
				.replace( /&/g, '&amp;' )
				.replace( /</g, '&lt;' )
				.replace( />/g, '&gt;' );
			...
			top =
				theme.close_position_top +
				( theme.close_position_outside
					? $( '#wpadminbar' ).outerHeight()
					: 0 );
			left =
				theme.close_position_left +
				( theme.close_position_outside
					? $( '#adminmenuwrap' ).outerWidth()
					: 0 );
	...
	$( document )
		...
		.on(
			'click',
			'.pum-popup-overlay, .pum-popup-container, .pum-popup-title, .pum-popup-content, .pum-popup-close',
			function ( event ) {
				var $this = $( this ),
				...
				switch ( clicked_class ) {
					case 'pum-popup-overlay':
						$(
							'a[href="#pum-theme-settings_overlay"]'
						)[ 0 ].click();
						break;
					case 'pum-popup-container':
						$(
							'a[href="#pum-theme-settings_container"]'
						)[ 0 ].click();
						break;
					...
					case 'pum-popup-content':
						$(
							'a[href="#pum-theme-settings_content"]'
						)[ 0 ].click();
						break;
				...
				$( 'html, body' ).animate( {
					scrollTop:
						$( '#pum_theme_settings' ).offset().top - 40 + 'px',
				} );
			}
		)
		/**
		 ...
		 */
		.on(
			'change colorchange input focusout',
			'.pum-field select, .pum-field input',
			function () {
