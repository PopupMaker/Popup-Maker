...
( function ( $ ) {
	...
	function check_provider() {
		var $provider = $( '#pum-shortcode-editor-pum_sub_form #provider' ),
			provider =
				$provider.val() !== '' && $provider.val() !== 'none'
					? $provider.val()
					: pum_admin_vars.default_provider,
			$provider_tabs = $(
				'.pum-modal-content .tabs .tab a[href^="#pum-shortcode-editor-pum_sub_form_provider_"]'
			),
			$provider_contents = $(
				'[id^="pum-shortcode-editor-pum_sub_form_provider_"]'
			),
			$selected_tab = $provider_tabs.filter(
				'[href="#pum-shortcode-editor-pum_sub_form_provider_' +
					provider +
					'"]'
			),
			$selected_contents = $provider_contents.filter(
				'[id="pum-shortcode-editor-pum_sub_form_provider_' +
					provider +
					'"]'
			);
	...
	$( document )
		...
		.on(
			'change',
			'#pum-shortcode-editor-pum_sub_form #provider',
			check_provider
		);
	...
	function check_list() {
		var $list_id = $(
				'#pum-shortcode-editor-pum_sub_form_provider_mailchimp #list_id'
			),
			list_id = $list_id.val(),
			$list_options = $(
				'#pum-mci-list-' + list_id + ',.pum-mci-list-' + list_id
			),
	...
	$( document )
		.on(
			'pumInit pum_init',
			'#pum-shortcode-editor-pum_sub_form',
			check_list
		)
		.on(
			'change',
