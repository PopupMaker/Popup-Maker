...
( function ( $ ) {
	...
	var I10n = pum_shortcode_ui_vars.I10n || {
		...
		base = {
			...
			getShortcodeValues: function () {
				if (
					typeof this.shortcode === 'undefined' ||
					typeof this.shortcode.attrs === 'undefined'
				) {
					return {};
				}
				...
				if ( typeof this.shortcode.attrs.numeric !== 'undefined' ) {
					for (
						var i = 0;
						i < this.shortcode.attrs.numeric.length;
						i++
					) {
			...
			formatShortcode: function ( values ) {
				...
				if (
					has_content &&
					typeof values._inner_content !== 'undefined'
				) {
				...
				if ( ! self.fetching ) {
					...
					if ( ! self.shortcode_args.ajax_rendering ) {
						...
						$.post( ajaxurl, data )
							...
							.fail( function () {
								self.content =
									'<span class="pum_shortcode_ui_vars_error">' +
									I10n.error_loading_shortcode_preview +
									'</span>';
							} )
							.always( function () {
								delete self.fetching;
			...
			renderForm: function ( values, callback ) {
				...
				PUM_Admin.modals.reload(
					...
					PUM_Admin.templates.modal( {
						...
						save_button:
							undefined === values ? I10n.insert : I10n.update,
				...
				$( '#' + data.id + ' form' ).on( 'submit', function ( event ) {
					...
					var $form = $( this ),
						...
						values = PUM_Admin.forms.parseValues(
							$form.pumSerializeObject().attrs,
							PUM_Admin.forms.flattenFields( data )
						),
						content;
	...
	$( document ).on(
		'pumFormDependencyMet pumFormDependencyUnmet',
		'.pum-shortcode-editor .pum-field',
		function ( event ) {
			...
			wp.mce.pum_shortcodes[ tag ] = _.extend( {}, base, {
				...
				View: {
					...
					template: function ( options ) {
						return wp.mce.pum_shortcodes[ this.type ].template(
							options
						);
					},
					postID: $( '#post_ID' ).val(),
					initialize: function ( options ) {
						...
						wp.mce.pum_shortcodes[ this.type ].shortcode_data =
							this.shortcode;
					},
					getHtml: function () {
