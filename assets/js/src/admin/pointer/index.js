	...
	function open_pointer( id ) {
		...
		var options = $.extend( pointer.options, {
			close: function () {
				...
				if (
					false === skipTour &&
					id !== pumPointers.pointers.length - 1
				) {
					open_pointer( id + 1 );
				}
			},
			buttons: function ( event, t ) {
				var $btn_skip = $(
					'<button class="button" style="margin-right: 10px;">Skip tour</button>'
				);
				var $btn_next = $(
					'<button class="button button-primary">Next</button>'
				);
				var $btn_complete = $(
					'<button class="button button-primary">Thanks!</button>'
				);
					...
					for ( var i = 0; i < pumPointers.pointers.length; i++ ) {
						...
						setTimeout(
							function ( i ) {
								$.post( ajaxurl, {
									pointer:
										pumPointers.pointers[ i ].pointer_id,
									action: 'dismiss-wp-pointer',
								} );
							},
							i * 500 + 1,
