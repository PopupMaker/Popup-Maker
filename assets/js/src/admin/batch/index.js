...
( function ( $ ) {
	...
	var batch = {
			form: {
				beforeSubmit: function ( $form ) {
					var $submit = $form.find(
							'.pum-field-submit input[type="submit"]'
						),
			...
			complete: function ( $form ) {
				...
				$alert
					.removeClass( 'pum-alert__warning' )
					.addClass( 'pum-alert__success' );
				...
				$.ajax( {
					...
					success: function ( response ) {
						if ( response.data.done || response.data.error ) {
							var batchSelector = response.data.mapping
									? '.pum-batch-import-form'
									: '.pum-batch-form',
							...
							if ( response.data.error ) {
								...
								spinner.remove();
								notice_wrap.html(
									'<div id="pum-batch-success" class="updated notice"><p class="pum-batch-success">' +
				...
				$.ajax( {
					...
					success: function ( response ) {
						...
						if ( response.data.done || response.data.error ) {
							...
							$( '.pum-batch-progress' ).addClass(
								'pum-batch-progress--active'
							);

							$(
								'.pum-batch-progress progress.pum-task-progress'
							)
								.addClass( 'active' )
	...
	$( document )
		.on( 'submit', '.pum-batch-form[data-batch_id]', function ( event ) {
			...
			if ( ! submitButton.hasClass( 'button-disabled' ) ) {
				...
				$this.append(
					$(
						'<div class="notice-wrap"><div class="pum-batch-progress"><div></div>'
					)
				);
		...
		.on(
			'submit',
			'.pum-batch-form.pum-upgrade-form[data-upgrade_id]',
			function ( event ) {
				var $form = $( this ),
					data = {
		...
		serializeAssoc: function () {
			var o = {
				...
				add: function ( name, value ) {
					...
					if ( tmp ) {
						if ( tmp[ 2 ] ) v[ tmp[ 2 ] ] = value;
						else v[ $.count( v ) ] = value;
						this.add( tmp[ 1 ], v );
					} else if ( typeof value === 'object' ) {
						if ( typeof this.aa[ name ] !== 'object' ) {
							this.aa[ name ] = {};
						}
						this.aa[ name ] = $.arrayMerge(
							this.aa[ name ],
							value
						);
					} else {
