		...
		templates = {
			...
			shortcode: function ( args ) {
				var data = $.extend(
					...
					template = data.has_content
						? 'pum-shortcode-w-content'
						: 'pum-shortcode';
			...
			tabs: function ( data ) {
				...
				data.classes.push(
					data.vertical ? 'vertical-tabs' : 'horizontal-tabs'
				);
			...
			fieldArgs: function ( args ) {
				...
				if (
					args.std !== undefined &&
					args.type !== 'checkbox' &&
					( data.value === null || data.value === false )
				) {
					data.value = args.std;
				}
				...
				if (
					...
					typeof args.dependencies === 'object'
				) {
				...
				if (
					typeof data.dynamic_desc === 'string' &&
					data.dynamic_desc.length
				) {
					data.classes.push( 'pum-field-dynamic-desc' );
					data.desc = PUM_Admin.templates.renderInline(
						data.dynamic_desc,
						data
					);
				}
				...
				if ( data.allow_html ) {
					...
					if (
						...
						data.value !== '' &&
						PUM_Admin.utils.htmlencoder.hasEncoded( data.value )
					) {
						data.value = PUM_Admin.utils.htmlencoder.htmlDecode(
							data.value
						);
					}
				}
				...
				switch ( args.type ) {
					...
					case 'taxonomyselect':
						...
						if ( args.type !== 'select' ) {
							...
							data.classes.push(
								args.type === 'postselect'
									? 'pum-field-postselect'
									: 'pum-field-taxonomyselect'
							);
							data.meta[ 'data-objecttype' ] =
								args.type === 'postselect'
									? 'post_type'
									: 'taxonomy';
							data.meta[ 'data-objectkey' ] =
								args.type === 'postselect'
									? args.post_type
									: args.taxonomy;
							data.meta[ 'data-current' ] =
					...
					case 'rangeslider':
						...
						data.meta[ 'data-force-minmax' ] =
							data.force_minmax.toString();
						break;
					case 'textarea':
					...
					case 'measure':
						if (
							...
							data.unitValue = data.value.replace(
								data.number,
								''
							);
					...
					case 'license_key':
						...
						data.classes.push(
							'pum-license-' + data.value.status + '-notice'
						);
			...
			field: function ( args ) {
				...
				if (
					data.type === 'objectselfect' ||
					data.type === 'postselect' ||
					data.type === 'taxonomyselect'
				) {
				...
				if ( ! $( '#tmpl-' + fieldTemplate ).length ) {
					console.warn(
						'No field template found for type:' +
							data.type +
							' fieldID: ' +
							data.id
					);
