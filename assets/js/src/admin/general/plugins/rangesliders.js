...
( function ( $ ) {
	...
	var rangesliders = {
		cloneables: {
			...
			plus: $(
				'<button type="button" class="pum-range-plus">+</button>'
			),
			minus: $(
				'<button type="button" class="pum-range-minus">-</button>'
			),
		},
		init: function () {
			$(
				'.pum-field-rangeslider:not(.pum-rangeslider-initialized)'
			).each( function () {
				...
				$slider
					.prop( {
						...
						max:
							settings.force ||
							( settings.max && settings.max > settings.value )
								? settings.max
								: settings.value * 1.5,
	...
	$( document )
		...
		.on(
			'input',
			'.pum-field-rangeslider.pum-rangeslider-initialized .pum-range-slider',
			function () {
				var $slider = $( this );
