...
( function ( $ ) {
		...
		modals = {
			...
			forceFocus: function ( event ) {
				if (
					...
					! PUM_Admin.modals._current.contains( event.target )
				) {
			...
			trapTabKey: function ( e ) {
				...
				if ( e.keyCode === 9 ) {
					...
					var focusableItems = PUM_Admin.modals._current
							.find( '*' )
	...
	$( document ).on(
		'click',
		'.pum-modal-background, .pum-modal-wrap .cancel, .pum-modal-wrap .pum-modal-close',
		function ( e ) {
			var $target = $( e.target );
			if (
				/*$target.hasClass('pum-modal-background') || */ $target.hasClass(
					'cancel'
				) ||
				$target.hasClass( 'pum-modal-close' ) ||
				$target.hasClass( 'submitdelete' )
			) {
				PUM_Admin.modals.closeAll();
				e.preventDefault();
				e.stopPropagation();
