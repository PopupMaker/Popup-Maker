...
( function ( $, document, undefined ) {
	...
	function isInt( value ) {
		return (
			! isNaN( value ) &&
			parseInt( Number( value ) ) === parseInt( value ) &&
	...
	window.PUM = {
		...
		getPopup: function ( el ) {
			...
			return $this.parents( '.pum-overlay' ).length
				? $this.parents( '.pum-overlay' )
				: $();
		},
		...
		disableClickTriggers: function ( el, trigger_settings ) {
			...
			if ( trigger_settings !== undefined ) {
				var selector = PUM.getClickTriggerSelector(
					el,
					trigger_settings
				);
				$( selector ).removeClass( 'pum-trigger' );
				$( document ).off(
					'click.pumTrigger click.popmakeOpen',
					selector
				);
			} else {
	...
	$.fn.popmake = function ( method ) {
		...
		if ( $.fn.popmake.methods[ method ] ) {
			...
			return $.fn.popmake.methods[ method ].apply(
				this,
		...
		if ( window.console ) {
			console.warn(
				'Method ' + method + ' does not exist on $.fn.popmake'
			);
		}
	};
	...
	$.fn.popmake.methods = {
		init: function () {
			return this.each( function () {
				...
				if (
					settings.disable_reposition === undefined ||
					! settings.disable_reposition
				) {
					$( window ).on( 'resize', function () {
						if (
							$popup.hasClass( 'pum-active' ) ||
							$popup.find( '.popmake.active' ).length
						) {
							$.fn.popmake.utilities.throttle(
								setTimeout( function () {
				...
				$popup.data( 'popmake', settings ).trigger( 'pumInit' );
				...
				if ( settings.open_sound && 'none' !== settings.open_sound ) {
					...
					var audio =
						'custom' !== settings.open_sound
							? new Audio(
									pum_vars.pm_dir_url +
										'assets/sounds/' +
										settings.open_sound
							  )
							: new Audio( settings.custom_sound );
		...
		getClose: function () {
			return (
				PUM.getPopup( this ).find( '.pum-content + .pum-close' ) || null
			);
		},
		getSettings: function () {
			...
			return $.extend(
				...
				$.fn.popmake.defaults,
				$popup.data( 'popmake' ) || {},
				typeof pum_popups === 'object' &&
					typeof pum_popups[ $popup.attr( 'id' ) ] !== 'undefined'
					? pum_popups[ $popup.attr( 'id' ) ]
					: {}
			);
		},
		state: function ( test ) {
		...
		setup_close: function () {
			...
			$close.off( 'click.pum' ).on( 'click.pum', function ( event ) {
				var $this = $( this ),
					do_default =
						$this.hasClass( 'pum-do-default' ) ||
						( $this.data( 'do-default' ) !== undefined &&
							$this.data( 'do-default' ) );
		...
		close: function ( callback ) {
			return this.each( function () {
				...
				if (
					$popup.hasClass( 'preventClose' ) ||
					$container.hasClass( 'preventClose' )
				) {
					$popup
		...
		reposition: function ( callback ) {
				...
				reposition = {
					...
					using:
						typeof callback === 'function'
							? callback
							: $.fn.popmake.callbacks.reposition_using,
				},
			...
			if ( settings.position_from_trigger && $last_trigger.length ) {
				...
				if ( location.indexOf( 'right' ) >= 0 ) {
					...
					reposition.at +=
						' right' +
						( settings.position_right !== 0
							? '+' + settings.position_right
							: '' );
				}
				if ( location.indexOf( 'center' ) >= 0 ) {
					reposition.my =
						location === 'center'
							? 'center'
							: reposition.my + ' center';
					reposition.at =
						location === 'center'
							? 'center'
							: reposition.at + ' center';
				}
				...
				if ( location.indexOf( 'bottom' ) >= 0 ) {
					...
					reposition.at +=
						' bottom' +
						( settings.position_bottom !== 0
							? '+' + settings.position_bottom
							: '' );
				}
			} else {
				if ( location.indexOf( 'left' ) >= 0 ) {
						...
						( settings.position_top !== 0
							? '+' +
							  ( $( 'body' ).hasClass( 'admin-bar' )
									? parseInt( settings.position_top, 10 ) + 32
									: settings.position_top )
							: '' );
					reposition.at += ' top';
				}
			...
			if ( settings.size === 'custom' ) {
				$container.css( {
					...
					height: settings.custom_height_auto
						? 'auto'
						: settings.custom_height,
				} );
			} else {
				if ( settings.size !== 'auto' ) {
					$container.addClass( 'responsive' ).css( {
						minWidth:
							settings.responsive_min_width !== ''
								? settings.responsive_min_width
								: 'auto',
						maxWidth:
							settings.responsive_max_width !== ''
								? settings.responsive_max_width
								: 'auto',
					} );
				}
