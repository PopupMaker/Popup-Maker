...
(function ($) {
    ...
    var conditions = {
        get_conditions: function () {
            return window.pum_message_editor.conditions_selectlist;
        },
        not_operand_checkbox: function ($element) {
            ...
            return $element.each(function () {
                ...
                PUM_Admin.conditions.toggle_not_operand($this);
            });
        ...
        template: {
            ...
            settings: function (args, values) {
                ...
                if (!data.fields.length && pum_message_editor.conditions[args.target] !== undefined) {
                    data.fields = pum_message_editor.conditions[args.target].fields;
                }
                ...
                _.each(data.fields, function (field, fieldID) {
                    ...
                    field.name = 'message_meta[conditions][' + data.group + '][' + data.index + '][settings][' + fieldID + ']';
            ...
            selectbox: function (args) {
                var data = $.extend(true, {}, {
                    ...
                    options: PUM_Admin.conditions.get_conditions()
                }, args);
                ...
                if (data.name === null) {
                    data.name = 'message_meta[conditions][' + data.group + '][' + data.index + '][target]';
                }
        ...
        groups: {
            add: function (editor, target, not_operand) {
                ...
                $editor.find('.facet-groups').append(PUM_Admin.conditions.template.group(data));
        ...
        facets: {
            add: function ($group, target, not_operand) {
                ...
                $group.find('.facet-list').append(PUM_Admin.conditions.template.facet(data));
            },
            remove: function ($facet) {
                ...
                if ($group.find('.facet').length === 0) {
                    PUM_Admin.conditions.groups.remove($group);
                } else {
                    PUM_Admin.conditions.renumber();
                }
            }
            ...
            $('.facet-builder .facet-group-wrap').each(function () {
                ...
                $group
                    ...
                    $facet
                        ...
                        var replace_with = "message_meta[conditions][" + groupIndex + "][" + facetIndex + "]";
                        this.name = this.name.replace(/message_meta\[conditions\]\[\d*?\]\[\d*?\]/, replace_with);
                        this.id = this.name;
                    });
                });
            });
    ...
    $(document)
        .on('pum_init', function () {
            PUM_Admin.conditions.renumber();
            PUM_Admin.conditions.toggle_not_operand();
        })
        .on('select2:select jpselect2:select', '#pum-first-condition', function (event) {
            var $field      = $(this),
            ...
            PUM_Admin.conditions.groups.add($editor, target, not_operand);
        ...
        .on('click', '.facet-builder .pum-not-operand', function () {
            PUM_Admin.conditions.not_operand_checkbox($(this));
        })
        .on('change', '.facet-builder .facet-target select', function (event) {
            ...
            $facet.data('target', target).find('.facet-settings').html(PUM_Admin.conditions.template.settings(data));
            $(document).trigger('pum_init');
        })
        .on('click', '.facet-builder .facet-group-wrap:last-child .and .add-facet', function () {
            PUM_Admin.conditions.groups.add($(this).parents('.facet-builder').eq(0));
            $(document).trigger('pum_init');
        })
        .on('click', '.facet-builder .add-or .add-facet:not(.disabled)', function () {
            PUM_Admin.conditions.facets.add($(this).parents('.facet-group-wrap').eq(0));
            $(document).trigger('pum_init');
        })
        .on('click', '.facet-builder .remove-facet', function () {
            PUM_Admin.conditions.facets.remove($(this).parents('.facet').eq(0));
            $(document).trigger('pum_init');
        });
...
(function ($, wpeditor_vars) {
    "use strict";

    var current_link_field,
        wp_editor;

    if (typeof tinyMCEPreInit === 'undefined' || typeof QTags === 'undefined' || typeof wpeditor_vars === 'undefined') {
    ...
    wp_editor = function (options) {
        ...
        default_options = {
            ...
            'mceInit': {
                ...
                "content_css": wpeditor_vars.includes_url + "css/dashicons.css?ver=3.9," + wpeditor_vars.includes_url + "js/mediaelement/mediaelementplayer.min.css?ver=3.9," + wpeditor_vars.includes_url + "js/mediaelement/wp-mediaelement.css?ver=3.9," + wpeditor_vars.includes_url + "js/tinymce/skins/wordpress/wp-content.css?ver=3.9",
                "selector": "#pum_id",
                "resize": "vertical",
                "menubar": false,
                "wpautop": true,
        ...
        if (tinyMCEPreInit.mceInit.pum_id) {
            default_options.mceInit = tinyMCEPreInit.mceInit.pum_id;
        }
        ...
        return this.each(function () {
            ...
            $this.before('<link rel="stylesheet" id="editor-buttons-css" href="' + wpeditor_vars.includes_url + 'css/editor.css" type="text/css" media="all">');
    ...
    // Import this module.
    window.PUM_Admin = window.PUM_Admin || {};
    window.PUM_Admin.wp_editor = wp_editor;

    $(document)
        .on('pum_init', function () {
            $('.pum-field-editor textarea:not(.initialized)').each(function () {
        ...
        .on('mousedown', '.pum-submit button', function (event) {
            var $form = $(event.target).parents('form').eq(0);

            tinyMCE.triggerSave();

            $form.trigger('pum_before_submit');
        })
        // Link Fields
        .on('click', '.pum-field-link button', function (event) {
            ...
            PUM_Admin.selectors('#wp-link-wrap').removeClass('has-text-field');
            PUM_Admin.selectors('#wp-link-target').hide();
            PUM_Admin.selectors('#pum-restriction-editor', true).hide();
        ...
        .on('click', '#wp-link-submit, #wp-link-cancel button, #wp-link-close', function (event) {
            ...
            // Show our editor
            PUM_Admin.selectors('#pum-restriction-editor').show();
...
}(jQuery, window.pum_wpeditor_vars || {}));
/*******************************************************************************
 * Copyright (c) 2017, WP Popup Maker
 ******************************************************************************/
(function ($) {
    ...
    window.pum_message_fields = {};

    if (typeof pum_message_editor !== 'undefined' && typeof pum_message_editor.meta_fields === 'object') {
        _.each(pum_message_editor.meta_fields, function (sectionFields, sectionID) {
            _.each(sectionFields, function (fieldArgs) {
                window.pum_message_fields[fieldArgs.id] = fieldArgs;
            });
        });
    }

    window.pum_message_editor = window.pum_message_editor || {
        form_args: {},
        current_values: {}
    };
    ...
    $(document)
        .ready(function () {
            var $container = $('#pum-message-settings-container'),
                args       = pum_message_editor.form_args || {},
                values     = pum_message_editor.current_values || {};
        ...
        .on('click', '#publish', function (event) {
            var $style           = $(':input[name="message_meta[style]"]:checked'),
                required_trigger = false,
                i,
                el;

            if ($style.val() === 'auto') {
                for (i = 0; i < pum_admin_vars.required_auto_triggers.length; i++) {
                    el = $('#' + pum_admin_vars.required_auto_triggers[i]);

                    if (el.length && ((el.attr('type') === 'checkbox' && el.is(':checked')) || (el.attr('type') !== 'checkbox' && el.val() !== ""))) {
                ...
                if (!required_trigger) {
                    event.preventDefault();
                    event.stopPropagation();
                    alert(pum_admin_vars.I10n.one_trigger_required);
                }
            }
        });
...
(function ($) {
    "use strict";
    window.pum_settings_editor = window.pum_settings_editor || {
        form_args: {},
        current_values: {}
    };
