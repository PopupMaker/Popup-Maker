!function (e) {
    ...
    void 0 === e.fn.on && (e.fn.on = function (e, t, o) {
        ...
        var t = e(this).clone(), o = e("<div/>").append(t);
        return o.html()
...
!function (e, t, o) {
    ...
    PUM = {
        ...
        init: function (t) {
            ...
            var n = PUM.getPopup(this), i = n.popmake("getContainer"), a = n.popmake("getClose"), s = n.popmake("getSettings");
            return s.meta.display.stackable || n.popmake("close_all"), n.addClass("pum-active").popmake("setup_close").popmake("reposition").trigger("pumBeforeOpen"), s.meta.close.button_delay > 0 && a.fadeOut(0), n.hasClass("preventOpen") || i.hasClass("preventOpen") ? (n.removeClass("preventOpen").removeClass("pum-active").trigger("pumOpenPrevented"), this) : (e("html").addClass("pum-open"), n.css({"z-index": s.meta.display.overlay_zindex || 1999999998}).popmake("animate", s.meta.display.animation_type, function () {
            ...
            return this.each(function () {
                ...
                return n.trigger("pumBeforeClose"), n.hasClass("preventClose") || i.hasClass("preventClose") ? (n.removeClass("preventClose").trigger("pumClosePrevented"), this) : (i.fadeOut("fast", function () {
                    n.is(":visible") && n.fadeOut("fast"), e(window).off("keyup.popmake"), n.off("click.popmake"), a.off("click.popmake"), e("html").removeClass("pum-open"), n.removeClass("pum-active").trigger("pumAfterClose"), i.find("iframe").filter('[src*="youtube"],[src*="vimeo"]').each(function () {
            ...
            var n, i, a = PUM.getPopup(this), s = a.popmake("getContainer"), p = a.popmake("getTitle"), r = a.popmake("getContent"), c = a.popmake("getClose"), m = a.popmake("getSettings");
            switch (t === o && (t = e.fn.popmake.themes[m.theme_id], t === o && (t = e.fn.popmake.themes[1])), n = "yes" === t.container.boxshadow_inset ? "inset " : "", i = "yes" === t.close.boxshadow_inset ? "inset " : "", a.removeAttr("style").css({
                backgroundColor: e.fn.popmake.utilities.convert_hex(t.overlay.background_color, t.overlay.background_opacity),
                zIndex: m.meta.display.overlay_zindex || 998
            }), s.css({
                ...
                zIndex: m.meta.display.zindex || 999
            }), p.css({
...
!function (e, t, o) {
    ...
    PUM_Accessibility = {
        forceFocus: function (t) {
            console.log(a, !e.contains(a, t.target)), a && !e.contains(a, t.target) && (t.stopPropagation(), PUM_Accessibility.setFocusToFirstItem())
        }, trapTabKey: function (t) {
            if (9 === t.keyCode) {
                var o = a.find(".pum-container *").filter(s).filter(":visible"), n = e(":focus"), i = o.length, p = o.index(n);
                console.log(o), t.shiftKey ? 0 === p && (o.get(i - 1).focus(), t.preventDefault()) : p === i - 1 && (o.get(0).focus(), t.preventDefault())
            }
        }, setFocusToFirstItem: function () {
            a.find(".pum-container *").filter(s).filter(":visible").first().focus()
        }
...
!function (e, t, o) {
    ...
    e.fn.popmake.last_open_trigger = null, e.fn.popmake.last_close_trigger = null, e.fn.popmake.conversion_trigger = null, PUM_Analytics = {
        send: function (t, n) {
            var i = new Image;
            t = e.extend({}, {action: "pum_analytics"}, t), t._cache = +new Date, n !== o && i.addEventListener("load", function () {
                n(t)
            }), i.src = pum_vars.ajaxurl + "?" + e.param(t)
        }
    }, e("body > .pum").on("pumAfterOpen.core_analytics", function () {
    ...
    e.fn.popmake.methods.animate_overlay = function (t, o, n) {
        ...
        none: function (e) {
            ...
            var t = PUM.getPopup(this).show(0).css({opacity: 0}), n = t.popmake("getContainer").show(0).css({opacity: 0}), i = t.popmake("getSettings"), a = i.meta.display.animation_speed / 2, s = t.popmake("animation_origin", i.meta.display.animation_origin);
            ...
            var t = PUM.getPopup(this), n = t.popmake("getContainer"), i = t.popmake("getSettings"), a = i.meta.display.animation_speed / 2;
...
!function (e, t, o) {
    ...
    e.fn.popmake.cookie = {
        defaults: {}, raw: !1, json: !0, pluses: /\+/g, encode: function (t) {
            ...
            var p, r, c, m, u, l = n ? o : {}, f = new Date, d = t.cookie ? t.cookie.split("; ") : [];
            ...
            for (m = 0, u = d.length; u > m; m += 1) {
                if (p = d[m].split("="), r = e.fn.popmake.cookie.decode(p.shift()), c = p.join("="), n && n === r) {
                    l = e.fn.popmake.cookie.read(c, i);
                    break
                }
                c = e.fn.popmake.cookie.read(c), n || c === o || (l[r] = c)
            }
            return l
        }, remove: function (t) {
    ...
    e.fn.popmake.methods.addTrigger = function (t) {
        ...
        auto_open: function (t) {
            ...
            var n = PUM.getPopup(this), i = n.popmake("getSettings"), a = ".popmake-" + i.id + ", .popmake-" + decodeURIComponent(i.slug);
            "" !== o.extra_selectors && (a += ", " + o.extra_selectors), e(a).addClass("pum-trigger").css({cursor: "pointer"}), e(t).on("click.pumTrigger", a, function (t) {
                n.has(this).length > 0 || n.popmake("checkCookies", o) || (e(t.target).hasClass("do-default") || (t.preventDefault(), t.stopPropagation()), e.fn.popmake.last_open_trigger = this, n.popmake("open"))
            })
    ...
    e.fn.popmake.utilities = {
        convert_hex: function (e, t) {
            ...
            function i(e) {
                var t = e.split(" "), o = t[0], i = t[1].substring(0, 3), a = /\d+/.test(o), s = "ago" === t[2], p = ("last" === o ? -1 : 1) * (s ? -1 : 1);
                if (a && (p *= parseInt(o, 10)), u.hasOwnProperty(i) && !t[1].match(/^mon(day|\.)?$/i))return c["set" + u[i]](c["get" + u[i]]() + p);
                if ("wee" === i)return c.setDate(c.getDate() + 7 * p);
                if ("next" === o || "last" === o)n(o, i, p); else if (!a)return !1;
                return !0
            }

            var a, s, p, r, c, m, u, l, f, d, g, h = !1;
            if (!e)return h;
            ...
            case"-":
                return s[3] > 12 || s[5] > 31 ? h : new Date(s[1], parseInt(s[3], 10) - 1, s[5], s[6] || 0, s[7] || 0, s[8] || 0, s[9] || 0) / 1e3;
            case".":
                return h;
            case"/":
                return s[3] > 12 || s[5] > 31 ? h : new Date(s[1], parseInt(s[3], 10) - 1, s[5], s[6] || 0, s[7] || 0, s[8] || 0, s[9] || 0) / 1e3
            ...
            case"-":
                return s[3] > 12 || s[1] > 31 ? h : new Date(s[5], parseInt(s[3], 10) - 1, s[1], s[6] || 0, s[7] || 0, s[8] || 0, s[9] || 0) / 1e3;
            case".":
                return s[3] > 12 || s[1] > 31 ? h : new Date(s[5], parseInt(s[3], 10) - 1, s[1], s[6] || 0, s[7] || 0, s[8] || 0, s[9] || 0) / 1e3;
            case"/":
                return s[1] > 12 || s[3] > 31 ? h : new Date(s[5], parseInt(s[1], 10) - 1, s[3], s[6] || 0, s[7] || 0, s[8] || 0, s[9] || 0) / 1e3
            } else switch (s[2]) {
            case"-":
                return s[3] > 12 || s[5] > 31 || s[1] < 70 && s[1] > 38 ? h : (r = s[1] >= 0 && s[1] <= 38 ? +s[1] + 2e3 : s[1], new Date(r, parseInt(s[3], 10) - 1, s[5], s[6] || 0, s[7] || 0, s[8] || 0, s[9] || 0) / 1e3);
            case".":
                return s[5] >= 70 ? s[3] > 12 || s[1] > 31 ? h : new Date(s[5], parseInt(s[3], 10) - 1, s[1], s[6] || 0, s[7] || 0, s[8] || 0, s[9] || 0) / 1e3 : s[5] < 60 && !s[6] ? s[1] > 23 || s[3] > 59 ? h : (p = new Date, new Date(p.getFullYear(), p.getMonth(), p.getDate(), s[1] || 0, s[3] || 0, s[5] || 0, s[9] || 0) / 1e3) : h;
            case"/":
                return s[1] > 12 || s[3] > 31 || s[5] < 70 && s[5] > 38 ? h : (r = s[5] >= 0 && s[5] <= 38 ? +s[5] + 2e3 : s[5], new Date(r, parseInt(s[1], 10) - 1, s[3], s[6] || 0, s[7] || 0, s[8] || 0, s[9] || 0) / 1e3);
            case":":
                return s[1] > 23 || s[3] > 59 || s[5] > 59 ? h : (p = new Date, new Date(p.getFullYear(), p.getMonth(), p.getDate(), s[1] || 0, s[3] || 0, s[5] || 0) / 1e3)
            }
            if ("now" === e)return null === t || isNaN(t) ? (new Date).getTime() / 1e3 || 0 : t || 0;
            if (a = Date.parse(e), !isNaN(a))return a / 1e3 || 0;
            if (c = t ? new Date(1e3 * t) : new Date, m = {
                    sun: 0,
                    mon: 1,
                    tue: 2,
                    wed: 3,
                    ...
                    min: "Minutes",
                    sec: "Seconds"
                }, f = "(years?|months?|weeks?|days?|hours?|minutes?|min|seconds?|sec|sunday|sun\\.?|monday|mon\\.?|tuesday|tue\\.?|wednesday|wed\\.?|thursday|thu\\.?|friday|fri\\.?|saturday|sat\\.?)", d = "([+-]?\\d+\\s" + f + "|(last|next)\\s" + f + ")(\\sago)?", s = e.match(new RegExp(d, "gi")), !s)return h;
            for (g = 0, l = s.length; l > g; g += 1)if (!i(s[g]))return h;
