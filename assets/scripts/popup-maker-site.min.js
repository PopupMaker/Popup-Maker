...
void 0!==a&&a()}}))})});return this}};c(".popmake").css({visibility:"visible"}).hide();c(document).ready(function(){c(".popmake").popmake().each(function(){var a=c(this),e=a.data("popmake"),d=e.meta.click_open,b=".popmake-"+e.id+", .popmake-"+e.slug,g=e.meta.admin_debug,f=e.meta.auto_open,h;void 0!==d.extra_selectors&&""!==d.extra_selectors&&(b+=", "+d.extra_selectors);c(b).css({cursor:"pointer"});c(document).on("click",b,function(b){b.preventDefault();b.stopPropagation();c.fn.popmake.last_open_trigger=
c.fn.popmake.utilities.getXPath(this);a.popmake("open")});if(void 0!==g&&g.enabled)a.popmake("open");else if(void 0!==f&&f.enabled){h=function(){return void 0===c.pm_cookie("popmake-auto-open-"+e.id+"-"+f.cookie_key)};a.on("popmakeSetCookie",function(){""!==f.cookie_time&&h()&&c.pm_cookie("popmake-auto-open-"+e.id+"-"+f.cookie_key,!0,f.cookie_time,f.cookie_path)});switch(f.cookie_trigger){case "open":a.on("popmakeAfterOpen",function(){a.trigger("popmakeSetCookie")});break;case "close":a.on("popmakeBeforeClose",
function(){a.trigger("popmakeSetCookie")})}setTimeout(function(){h()&&(c.fn.popmake.last_open_trigger="Auto Open Popups ID-"+e.id,a.popmake("open"))},f.delay)}})})})(jQuery);