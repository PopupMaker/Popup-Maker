...
d.parents(".options");a=jQuery('.tabs-panel-active input[type="checkbox"]:checked',c);b=jQuery("textarea",c);e=jQuery(".tagchecklist",c);g=b.val().split(",");for(c=0;c<g.length;c+=1)g[c]=parseInt(g[c],10);a.each(function(){k=jQuery(this);h=parseInt(k.val(),10);f=k.parent("label").siblings(".menu-item-title").val();-1===jQuery.inArray(h,g)&&g.push(h);e.append('<span><a class="ntdelbutton" data-id="'+h+'">X</a> '+f+"</span>")});b.text(g.join(","))}else d.hasClass("ntdelbutton")&&(k=d,n=parseInt(k.data("id"),
10),c=d.parents(".options"),b=jQuery("textarea",c),e=jQuery(".tagchecklist",c),g=b.val().split(","),g=jQuery.grep(g,function(a){return parseInt(a,10)!==parseInt(n,10)}),k.parent("span").remove(),b.text(g.join(",")))})},attachQuickSearchListeners:function(){var a;jQuery(".quick-search").keypress(function(b){var c=jQuery(this);if(13===b.which)return PopMakeAdmin.updateQuickSearchResults(c),!1;a&&clearTimeout(a);a=setTimeout(function(){PopMakeAdmin.updateQuickSearchResults(c)},400)}).attr("autocomplete",
