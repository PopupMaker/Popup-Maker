...
class Divi_Builder_Test extends WP_UnitTestCase {
	...
	public function test_support_and_editor_filters_stay_scoped_to_popups() {
		...
		$this->assertTrue( $provider->force_classic_editor( true, get_post( $control_popup_id ) ) );
		$this->assertFalse( $provider->force_classic_editor( true, get_post( $divi_popup_id ) ) );
		$this->assertTrue( $provider->force_classic_editor( true, 'popup' ) );
		...
		try {
			...
			$this->assertNotFalse( has_filter( 'use_block_editor_for_post', [ $provider, 'force_classic_editor' ] ) );
		} finally {
			...
			remove_filter( 'use_block_editor_for_post', [ $provider, 'force_classic_editor' ], 999 );
		}
	}
