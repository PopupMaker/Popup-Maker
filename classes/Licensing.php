...
class PUM_Licensing {
	 ...
	 *
	 * @param object|bool|null $license Value status to check.
	 *
	 * @return bool
	 */
	 ...
	 *
	 * @param object|bool|null $license Value to check.
	 *
	 * @return bool
	 */
	 ...
	 *
	 * @param object|bool|null $license Expiration value to check.
	 *
	 ...
	 *
	 * @param object|bool|null $license Status value to check.
	 *
		...
		if ( self::has_license( $license ) ) {
			...
			if ( false === $license->success ) {
				...
				switch ( $license->error ) {
					case 'expired':
						$messages[] = sprintf( /* translators: 1. date of expiration */__( 'Your license key expired on %1$s. Please %2$srenew your license key%3$s.', 'popup-maker' ), date_i18n( get_option( 'date_format' ), strtotime( $license->expires, current_time( 'timestamp' ) ) ), '<a target="_blank" href="https://wppopupmaker.com/checkout/?edd_license_key=' . $key . '&utm_campaign=Licensing&utm_source=plugin-settings-page-licenses-tab&utm_medium=expired&utm_content=pum_license">', '</a>' );
						break;
					case 'disabled':
					case 'revoked':
						$messages[] = sprintf( /* translators: 1. string, 2 string */__( 'Your license key has been disabled. Please %1$scontact support%2$s for more information.', 'popup-maker' ), '<a target="_blank" href="https://wppopupmaker.com/support/?utm_campaign=Licensing&utm_source=plugin-settings-page-licenses-tab&utm_content=pum_license&utm_medium=revoked">', '</a>' );
						break;
					case 'missing':
						$messages[] = sprintf( /* translators: 1. string, 2 string to visit account */__( 'Invalid license. Please %1$svisit your account page%2$s and verify it.', 'popup-maker' ), '<a target="_blank" href="https://wppopupmaker.com/your-account/license-keys/?utm_campaign=Licensing&utm_source=plugin-settings-page-licenses-tab&utm_content=pum_license&utm_medium=missing">', '</a>' );
						break;
					case 'invalid':
					case 'site_inactive':
						$messages[] = sprintf( /* translators: 1. license key, 2 string, 3 string to visit account */__( 'Your %1$s is not active for this URL. Please %2$svisit your account page%3$s to manage your license key URLs.', 'popup-maker' ), Popup_Maker::$NAME, '<a target="_blank" href="https://wppopupmaker.com/your-account/license-keys/?utm_campaign=Licensing&utm_source=plugin-settings-page-licenses-tab&utm_content=pum_license&utm_medium=invalid">', '</a>' );
						break;
					case 'item_name_mismatch':
						$messages[] = sprintf( /* translators: 1. item name */__( 'This appears to be an invalid license key for %s.', 'popup-maker' ), Popup_Maker::$NAME );
						break;
					case 'no_activations_left':
						$messages[] = sprintf( /* translators: 1. string, 2 string */__( 'Your license key has reached its activation limit. %1$sView possible upgrades%2$s now.', 'popup-maker' ), '<a target="_blank" href="https://wppopupmaker.com/your-account/license-keys/?utm_campaign=Licensing&utm_source=plugin-settings-page-licenses-tab&utm_content=pum_license&utm_medium=no-activations-left">', '</a>' );
						break;
					case 'license_not_activable':
					...
					default:
						...
						$messages[] = sprintf( /* translators: 1. license key, 2 string, 3 string */__( 'There was an error with this license key: %1$s. Please %2$scontact our support team%3$s.', 'popup-maker' ), $error, '<a target="_blank" href="https://wppopupmaker.com/support/?utm_campaign=Licensing&utm_source=plugin-settings-page-licenses-tab&utm_content=pum_license&utm_medium=error-contact-support">', '</a>' );
						break;
				}
			} else {
				...
				switch ( $license->license ) {
					...
					default:
						...
						if ( 'lifetime' === $license->expires ) {
							...
							$messages[] = sprintf( /* translators: 1. expiration date, 2 string, 3 string */__( 'Your license key expires soon! It expires on %1$s. %2$sRenew your license key%3$s.', 'popup-maker' ), date_i18n( get_option( 'date_format' ), strtotime( $license->expires, current_time( 'timestamp' ) ) ), '<a target="_blank" href="https://wppopupmaker.com/checkout/?edd_license_key=' . $key . '&utm_campaign=Licensing&utm_source=plugin-settings-page-licenses-tab&utm_content=Popup+Maker+license&utm_medium=renew">', '</a>' );
						} else {
							$messages[] = sprintf( /* translators: 1. expiration date */__( 'Your license key expires on %s.', 'popup-maker' ), date_i18n( get_option( 'date_format' ), strtotime( $license->expires, current_time( 'timestamp' ) ) ) );
						}
						break;
				}
			}
		} else {
			$messages[] = sprintf( /* translators: 1. plugin name */__( 'To receive updates, please enter your valid %s license key.', 'popup-maker' ), Popup_Maker::$NAME );
	 ...
	 *
	 * @param object|bool|null $license Status to check.
	 *
	 * @return bool|string
	 */
